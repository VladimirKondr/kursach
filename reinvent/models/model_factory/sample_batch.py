...
class SampleBatch:
    ...
    smilies: List[str] | None = None  # processed SMILES
    states: np.ndarray | None = None  # states for items2
    
    # IMPALA-specific fields (optional)
    sequences: List[torch.Tensor] | None = None  # token ID sequences for each sample
    token_log_probs: List[torch.Tensor] | None = None  # log probs for each token in each sequence
    ...
    def __next__(self):
        ...
        try:
            ...
            # Handle items1 being None (for Reinvent without IMPALA fields)
            items1_value = self.items1[idx] if self.items1 is not None else None
